...
public class instantiateAirplanesTraining : MonoBehaviour
{
	...
	public float timeBetweenPlanesMax = 4f;
	...
	void FixedUpdate ()
	{
		if (spawn_air) {
			if (Time.realtimeSinceStartup > secondsBeforeFirstPlaneAppears && Time.time >= timeLastPlane) {
				...
				float crashpositionx = crashpoints [0] [0];
				float crashpositiony = crashpoints [0] [1];
